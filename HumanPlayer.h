...
public:
	HumanPlayer(bool c) : Player(true) {}//建構
	~HumanPlayer() {};//解構
...
void HumanPlayer::KingCanMove(Board& const board, int y, int x, bool c) {
	//判斷是否可移動
	//九宮格
...
void HumanPlayer::QueenCanMove(Board& const board, int y, int x, bool c) {
	//判斷是否可移動
...
void HumanPlayer::BishopCanMove(Board& const board, int y, int x, bool c) {
	bool isBlock = false;//被阻擋
			//判斷是否可移動
	for (int i = 0; i < 8; i++) {
		if (y - i >= 0 && x - i >= 0) canMove[y - i][x - i] = true;//左上
...
void HumanPlayer::KnightCanMove(Board& const board, int y, int x, bool c) {
	//判斷是否可移動
...
void HumanPlayer::RookCanMove(Board& const board, int y, int x, bool c) {
	bool isBlock = false;//被阻擋
	//判斷是否可移動
	for (int i = 0; i < 8; i++) {
		canMove[i][x] = true;//直
...
void HumanPlayer::ParwnCanMove(Board& const board, int y, int x, bool c) {
...
int HumanPlayer::OnMove(Board& const board, int y, int x) {
	int chess = board.position[y][x] * -1;
	bool color = true;
	canmove = false;
...
void HumanPlayer::Moving(Board& const board, int chess, int to_y, int to_x) {
	//不可移動
...
void HumanPlayer::OnPromote(Board& const board, int chess, int to_y, int to_x) {
	int chosenchess;
	cout << "將將將~~恭喜~~\n";
	cout << "選擇要升級成什麼棋子\n";
	cin >> chosenchess;
